...
function setOverlayImageSize() {
    $(document).imagesLoaded(function () {
        ...
        $('.image-box-image').closest('.image-overlay-box').css('height', (imageHeight / 2) + parseInt(sectionPadding) + 'px');
...
function setMenuImageSize() {
    if ($(document).width() >= 767) {
        ...
        var wrapperPadding = parseInt($('.menu-feature-box .menu-feature-box-wrapper').css('padding-left'));
        var imageMaxWidth = $('.menu-feature-box').outerWidth() - (boxPadding);
        $('.menu-feature-box .box-image img').css({'max-width': imageMaxWidth, 'transform': "translateX(-" + (boxPadding + wrapperPadding) + "px)"});
    } else {
        $('.menu-feature-box .box-image img').css({'max-width': "", 'transform': ""});
    }
}
